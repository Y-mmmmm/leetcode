...
public:
    Node* copyRandomList(Node* head) {
        ...
        while(cur)
        {
            cur->next = cur->next->next; //先将原链表连接起来
            cur = cur->next;//然后移动到原链表的下一个节点

            //复制节点的下一个不为空，也就是到末尾为止
            if(curCopy->next != nullptr)
            {
                curCopy->next = curCopy->next->next; //将复制节点连接起来
                curCopy = curCopy->next; //然后移动指针到复制节点的下一个节点
            }
        }
        return copyHead;
    }
