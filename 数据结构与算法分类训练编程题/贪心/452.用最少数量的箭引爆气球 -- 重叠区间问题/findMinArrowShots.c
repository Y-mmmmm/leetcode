...
int findMinArrowShots(int** points, int pointsSize, int* pointsColSize){
    if(points == NULL || pointsSize < 1)    return 0;

    qsort(points,pointsSize,sizeof(int *),cmp); //注意 int *

    int x_end = points[0][1];

    int nonOverLapCnt = 1; //从1开始

    for(int i=1;i<pointsSize;i++)
    {
        int start = points[i][0];
        if(start > x_end) //注意这里就是 > ，因为边界相同也会爆
        {
            nonOverLapCnt++;
            x_end = points[i][1];
        }
